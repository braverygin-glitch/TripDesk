window.BookingsFeature = {
  ...
  itemHtml(item) {
    ...
    const cleanMemo = this.cleanMemo(item.memo, item.reservationNo, item.address);
      ...
      <div class="item booking-item">
        <div class="row-between">
          <button class="booking-main" onclick="BookingsFeature.showForm('${item.id}')">
            ...
            ${cleanMemo ? `<div class="item-meta">📝 ${Utils.escape(cleanMemo)}</div>` : ""}
          </button>
  ...
  cleanMemo(memo, reservationNo = "", address = "") {
    let text = String(memo || "").trim();
    if (!text) return "";

    const removeParts = [];

    if (reservationNo) {
      removeParts.push(`예약번호: ${reservationNo}`);
      removeParts.push(`예약 번호: ${reservationNo}`);
      removeParts.push(`예약번호 ${reservationNo}`);
      removeParts.push(`예약 번호 ${reservationNo}`);
    }

    if (address) {
      removeParts.push(`주소: ${address}`);
      removeParts.push(`주소 ${address}`);
    }
    ...
    text = text
      .split(/\r?\n/)
      .map(line => line.trim())
      .filter(line => line && line !== "예약번호:" && line !== "주소:")
      .join("\n")
      .trim();
